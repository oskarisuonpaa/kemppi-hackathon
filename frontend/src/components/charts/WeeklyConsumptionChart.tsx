...
const WeeklyConsumptionChart: React.FC<TrendlineChartProps> = ({ data, metric }) => {
  ...
  const chartData = Object.keys(aggregatedData).map((weekStart) => {
    ...
    return {
      ...
      value: Number(aggregatedData[weekStart].toFixed(1)), // Format to one decimal place
    };
  });
  ...
  return (
    <div style={{ width: '49%' }}>
      <h3 style={{ fontSize: '1.5em', marginBottom: '10px' }}>{`Weekly Consumption Chart - ${yAxisLabel[metric]}`}</h3>
      <ResponsiveContainer width="100%" height={400}>
        <BarChart data={chartData}>
          ...
          <Legend />
          <Bar dataKey="value" fill="#f57300" name={yAxisLabel[metric]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
